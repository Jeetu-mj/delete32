...
router.put('/UpdateArticle/:id', upload.single('file'), async (req, res) => {
    try {
        console.log("Inside Product Article Update")
      const id = req.params.id;
      const notifications = req.body.notifications || [];
  
      // Find the overview details by examName
      const overviewData = await ProArtiDetails.findOne({ id });
  
      if (!overviewData) {
        return res.status(404).json({ error: "Overview details not found for the provided College Overview." });
      }
