...
function ManageCourse() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/getcourses');
        if (response.status === 200) {
          const newsdetails = response.data[0];
          console.log("Course Details",newsdetails)
  ...
  const handleStatusChange=async(row)=>{
    try {
      ...
      const response = await axios.put(`http://localhost:4000/api/updatecoursestatus/${row.id}`);
    ...
    const column = [
      ...
      const handlefilter = (event)=>{
        const newData = filterrec.filter(row=> row.title.toLowerCase().includes(event.target.value.toLowerCase()));
